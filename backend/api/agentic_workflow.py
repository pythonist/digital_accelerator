...
def _chat_json(session_id: str, stage: str, system: str, user: str, temperature: float = 0.1) -> dict:
    response = chatgpt_complete_json(system, user, temperature=temperature)
    _save_last_llm(session_id, stage, f"SYSTEM:\n{system}\n\nUSER:\n{user}", response)
    return response


def _chat_text(session_id: str, stage: str, system: str, user: str, temperature: float = 0.2) -> str:
    response = chatgpt_complete(system, user, temperature=temperature)
    _save_last_llm(session_id, stage, f"SYSTEM:\n{system}\n\nUSER:\n{user}", response)
    return response
...
def plan_investigation(session_id: str, case_id: str, context: dict, prior_memory: str = "") -> list[dict]:
    ...
    system = (
        "You are ChatGPT acting as the Planner Agent for a senior AML investigation. "
        "Use the supplied case data and available tools to produce a dynamic plan. "
        "Do not choose tools mechanically. Include only steps that are relevant to "
        "the evidence and explain why each step is needed. Respond only with JSON "
    ...
    result = _chat_json(session_id, "planner", system, user)
...
def execute_step(session_id: str, case_id: str, tool_name: str, step_db_id: str, context: dict) -> None:
    ...
    try:
        ...
        summary = nemotron_summarize(tool_name, raw_output, current_memory=memory_text)
        _save_last_llm(session_id, f"nemotron_summary:{tool_name}", raw_output[:20000], summary)
...
def synthesize_risk(session_id: str, case_id: str, memory_text: str) -> dict:
    system = (
        "You are ChatGPT acting as the Risk Agent. Produce an evidence-backed AML "
    ...
    return _chat_json(session_id, "risk_agent", system, user, temperature=0.1)


def qa_review(session_id: str, case_id: str, memory_text: str, risk_result: dict) -> dict:
    system = (
        "You are ChatGPT acting as the QA Reflection Agent. Decide whether the "
    ...
    return _chat_json(session_id, "qa_reflection", system, user, temperature=0.1)


def extract_findings(session_id: str, case_id: str, tool_log: list[dict], risk_result: dict) -> dict:
    evidence_blocks = "\n\n".join(
        f"=== {row.get('tool_name')} ({row.get('status')}) ===\n{row.get('output_data') or row.get('summary') or ''}"
        for row in tool_log
    )
    system = (
        "You are ChatGPT acting as the lead AML Findings Agent. Analyze the full "
    ...
    data = _chat_json(session_id, "findings_agent", system, user, temperature=0.12)
...
def generate_document(session_id: str, case_id: str, doc_type: str, findings_data: dict, risk_result: dict, tool_log: list[dict]) -> str:
    system = (
        "You are ChatGPT acting as the Report Agent for an enterprise AML team. "
    ...
    return _chat_text(session_id, f"document:{doc_type}", system, user, temperature=0.18)


def generate_reports(session_id: str, case_id: str, risk_result: dict) -> dict:
    tool_log = get_tool_execution_log(session_id)
    findings_data = extract_findings(session_id, case_id, tool_log, risk_result)
    save_findings(session_id, findings_data)
    for doc_type in DOCUMENT_TYPES:
        content = generate_document(session_id, case_id, doc_type, findings_data, risk_result, tool_log)
...
def run_agentic_investigation(case_id: str, session_id: str, context: dict | None = None) -> None:
    ...
    try:
        ...
        log_activity(
            session_id,
            f"LLM providers: ChatGPT={providers['chatgpt']['model']} Nemotron={providers['nemotron']['model']}",
            "info",
        )
        update_memory(session_id, f"Case ID: {case_id}\nInvestigation initialized at {datetime.utcnow().isoformat()}Z.")

        log_activity(session_id, "Planner Agent analyzing live case data with ChatGPT.", "running")
        steps = plan_investigation(session_id, case_id, context)
        step_ids = _insert_plan_steps(session_id, steps)
        log_activity(session_id, f"ChatGPT generated {len(steps)} investigation steps.", "success")
        ...
        for loop_index in range(MAX_QA_LOOPS + 1):
            memory_text = _current_memory(session_id)
            log_activity(session_id, "Risk Agent synthesizing evidence with ChatGPT.", "running")
            risk_result = synthesize_risk(session_id, case_id, memory_text)
            _finalize_risk(session_id, risk_result)
            log_activity(session_id, "Risk synthesis complete.", "success")

            log_activity(session_id, "QA Reflection Agent reviewing evidence sufficiency.", "running")
            qa_result = qa_review(session_id, case_id, memory_text, risk_result)
        ...
        log_activity(session_id, "Findings Agent analyzing the complete evidence trail.", "running")
        update_session_status(session_id, "running", current_step="Generating Findings and Documents")
        findings_data = generate_reports(session_id, case_id, risk_result)
        log_activity(
            session_id,
