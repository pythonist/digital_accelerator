...
class TypologyRulesEngine:
    ...
    def evaluate(self, profile_payload: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        ...
        if raw.get("pass_through_ratio", 0) >= 0.72:
            mule.append(self._signal("Pass-through ratio", f"{raw.get('pass_through_ratio', 0):.2f}", "Rapid release of incoming funds can indicate an account is being used to move third-party funds rather than retain them.", "transaction_behavior", "mule_account_behavior", 0.34))
        ...
        if raw.get("off_hours_ratio", 0) >= 0.25 or raw.get("weekend_ratio", 0) >= 0.25:
            mule.append(self._signal("Unusual transaction timing", f"Off-hours {raw.get('off_hours_ratio', 0):.2f} | Weekend {raw.get('weekend_ratio', 0):.2f}", "Unusual timing can strengthen concern that the account is being operated for suspicious movement rather than routine customer use.", "time_pattern_anomalies", "mule_account_behavior", 0.08))
        ...
        if raw.get("customer_risk_rating", 0) >= 60 or raw.get("pep_flag", 0) or raw.get("sanctions_flag", 0):
            mule.append(self._signal("Customer/account risk context", f"Risk {raw.get('customer_risk_rating', 0):.0f}", "Elevated customer risk can increase the seriousness of mule-like movement patterns.", "customer_account_risk", "mule_account_behavior", 0.08))
        ...
        if txn.get("sub_threshold_ratio", 0) >= 0.3:
            structuring.append(self._signal("Below-threshold transfer ratio", f"{txn.get('sub_threshold_ratio', 0):.2f}", "Repeated values just below common reporting thresholds can suggest threshold avoidance behavior.", "transaction_behavior", "structuring", 0.3))
        ...
        if raw.get("burstiness", 0) >= 0.35:
            structuring.append(self._signal("Burstiness", f"{raw.get('burstiness', 0):.2f}", "Concentrated transaction timing can support a structuring interpretation when sub-threshold values arrive in a short window.", "time_pattern_anomalies", "structuring", 0.1))
        if txn.get("round_amount_ratio", 0) >= 0.25:
            structuring.append(self._signal("Round amount ratio", f"{txn.get('round_amount_ratio', 0):.2f}", "Repeated rounded values can indicate deliberate transaction shaping rather than natural commerce.", "transaction_behavior", "structuring", 0.12))
        if raw.get("structuring_score", 0) >= 0.5 or "STRUCT" in str(metadata.get("dominant_alert_family") or "").upper():
            structuring.append(self._signal("Rule / profile alignment", f"{raw.get('structuring_score', 0):.2f}", "Existing rule or profile alignment strengthens the structuring hypothesis.", "rule_engine_outputs", "structuring", 0.16))
        if raw.get("suspicious_txn_count", 0) >= 6:
            structuring.append(self._signal("Suspicious transaction volume", f"{raw.get('suspicious_txn_count', 0):.0f}", "Higher suspicious transaction count can strengthen a repeated-threshold-avoidance pattern.", "alert_profile", "structuring", 0.08))
        ...
        if raw.get("high_risk_geo_ratio", 0) >= 0.25:
            layering.append(self._signal("High-risk geography exposure", f"{raw.get('high_risk_geo_ratio', 0):.2f}", "Routing through higher-risk geographies can strengthen layering concern.", "transaction_behavior", "layering", 0.12))
        if raw.get("layering_score", 0) >= 0.5:
            layering.append(self._signal("Profile layering score", f"{raw.get('layering_score', 0):.2f}", "Existing case-profile alignment increases the likelihood of a layering-style pattern.", "rule_engine_outputs", "layering", 0.16))
        if graph.get("network_risk_score", 0) >= 60:
            layering.append(self._signal("Network risk context", f"{graph.get('network_risk_score', 0):.0f}", "Higher network risk strengthens the significance of intermediary routing findings.", "network_graph_features", "layering", 0.08))
        ...
        if txn.get("dominant_counterparty_ratio", 0) >= 0.35:
            funnel.append(self._signal("Counterparty concentration", f"{txn.get('dominant_counterparty_ratio', 0):.2f}", "A high share of activity routed to a limited destination set can support funnel behavior.", "transaction_behavior", "funnel_account", 0.18))
        if raw.get("unique_counterparties", 0) >= 6:
            funnel.append(self._signal("Broad counterparty footprint", f"{raw.get('unique_counterparties', 0):.0f}", "A wide set of counterparties feeding a smaller destination set can indicate funneling.", "transaction_behavior", "funnel_account", 0.12))
        if raw.get("funnel_score", 0) >= 0.45:
            funnel.append(self._signal("Profile funnel score", f"{raw.get('funnel_score', 0):.2f}", "Case-profile alignment supports a funnel interpretation.", "rule_engine_outputs", "funnel_account", 0.16))
        ...
        if raw.get("pass_through_ratio", 0) >= 0.78:
            pass_through.append(self._signal("High pass-through ratio", f"{raw.get('pass_through_ratio', 0):.2f}", "High matched inflow and outflow suggests funds are not being retained for ordinary usage.", "transaction_behavior", "pass_through_behavior", 0.38))
        if raw.get("inbound_outbound_imbalance", 0) <= 0.25:
            pass_through.append(self._signal("Low inbound/outbound imbalance", f"{raw.get('inbound_outbound_imbalance', 0):.2f}", "Balanced inflow and outflow can support pass-through usage rather than retained customer activity.", "transaction_behavior", "pass_through_behavior", 0.12))
        if raw.get("burstiness", 0) >= 0.3:
            pass_through.append(self._signal("Condensed timing", f"{raw.get('burstiness', 0):.2f}", "Rapidly sequenced activity can strengthen a pass-through interpretation.", "time_pattern_anomalies", "pass_through_behavior", 0.1))
        if graph.get("collector_count", 0) or graph.get("distributor_count", 0):
        ...
        if raw.get("pass_through_typology_score", 0) >= 0.4:
            pass_through.append(self._signal("Profile pass-through score", f"{raw.get('pass_through_typology_score', 0):.2f}", "Existing case-profile alignment increases confidence in the pass-through hypothesis.", "rule_engine_outputs", "pass_through_behavior", 0.12))
        ...
        if raw.get("layering_score", 0) >= 0.45:
            circular.append(self._signal("Layering overlap", f"{raw.get('layering_score', 0):.2f}", "Circular movement can overlap with layering-style routing behavior.", "rule_engine_outputs", "circular_movement", 0.08))
        ...
        corridor = []
        if raw.get("high_risk_geo_ratio", 0) >= 0.25:
            corridor.append(self._signal("High-risk geography ratio", f"{raw.get('high_risk_geo_ratio', 0):.2f}", "Repeated exposure to higher-risk destinations can support corridor concern.", "transaction_behavior", "high_risk_corridor", 0.46))
        if raw.get("corridor_exposure", 0) >= 2:
            corridor.append(self._signal("Corridor breadth", f"{raw.get('corridor_exposure', 0):.0f}", "Multiple corridors can strengthen concern when combined with higher-risk destinations.", "transaction_behavior", "high_risk_corridor", 0.14))
        ...
        if raw.get("counterparty_concentration", 0) >= 0.35:
            beneficiary.append(self._signal("Counterparty concentration", f"{raw.get('counterparty_concentration', 0):.2f}", "High concentration suggests a limited destination set is receiving a large share of activity.", "transaction_behavior", "concentrated_beneficiary_pattern", 0.32))
        if txn.get("dominant_counterparty_ratio", 0) >= 0.4:
            beneficiary.append(self._signal("Dominant beneficiary ratio", f"{txn.get('dominant_counterparty_ratio', 0):.2f}", "A single beneficiary dominating visible flows can support concentration-based concern.", "transaction_behavior", "concentrated_beneficiary_pattern", 0.22))
        if raw.get("repeated_beneficiary_ratio", 0) >= 0.3:
            beneficiary.append(self._signal("Repeated beneficiary ratio", f"{raw.get('repeated_beneficiary_ratio', 0):.2f}", "Repeated reuse of the same destination accounts can strengthen beneficiary concentration findings.", "transaction_behavior", "concentrated_beneficiary_pattern", 0.12))
        if graph.get("collector_count", 0) > 0:
        ...
        if raw.get("burstiness", 0) >= 0.45:
            burst.append(self._signal("Burstiness", f"{raw.get('burstiness', 0):.2f}", "Strong activity concentration in a short time window can indicate non-routine suspicious movement.", "time_pattern_anomalies", "burst_spike_transaction_pattern", 0.4))
        if raw.get("off_hours_ratio", 0) >= 0.3 or raw.get("weekend_ratio", 0) >= 0.3:
            burst.append(self._signal("Time-pattern anomaly", f"Off-hours {raw.get('off_hours_ratio', 0):.2f} | Weekend {raw.get('weekend_ratio', 0):.2f}", "Unusual timing can strengthen a burst or spike interpretation.", "time_pattern_anomalies", "burst_spike_transaction_pattern", 0.12))
        if raw.get("suspicious_txn_count", 0) >= 8:
            burst.append(self._signal("High suspicious volume", f"{raw.get('suspicious_txn_count', 0):.0f}", "Higher suspicious transaction count can strengthen the reading of concentrated surge activity.", "alert_profile", "burst_spike_transaction_pattern", 0.14))
